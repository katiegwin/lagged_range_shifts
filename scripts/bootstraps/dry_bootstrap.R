...
###               CODE FOR CLUSTER BOOTSTRAP DRY EDGE
###                                                                          ###
################################################################################
################################################################################
...
load('edge/dryhypodata.RData') # cluster run only
...
# empty data frames: 
pred <- expand.grid(pptrange_fac = levels(trailingdry$pptrange_fac), temprange_pos = 0, canopy = "open", 
...
# the looooop
for (i in 1:nreps) {
  save(pred, file = 'edge/dryedge_pred17JAN24.RData') #intermittent save if case crashes
  save(warnings, file = 'edge/dryedge_warnings17JAN24.RData') # intermittent save if crashes
  print(paste0("Running model ", i)) # keep track of progress
  newmodel <- quietly(safely((refit)))(mdry, simulate(mdry)[[1]]) # bootstrap run that saves warnings (quietly) AND errors (safely) - warnings are convergence issues and errors are fatal (e.g., entire simulated dataset are zeroes)

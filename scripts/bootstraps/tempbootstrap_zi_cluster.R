...
###               CODE FOR CLUSTER BOOTSTRAP TEMP - ZERO INFLATED           ###
###                                                                          ###
################################################################################
################################################################################
...
# code for bootstrap run copied from range_position_analysis.R for cluster - zero inflated part
...
nreps <- 10001 # number of reps - note that lots of these reps will not converge

# empty data frames: 
optima <- data.frame(opt_temprange = rep(NA, nreps), opt_probzero = rep(NA, nreps)) # for optima - note opt_probzero is currently the probability of a zero need to flip later on
...
# the looooop - optimal thermal range position for zero inflation part of model
# note that we are extracting the min of peak because it plots it upsidedown, will fix later
for (i in 1:nreps) {
  save(pred, file = 'temprange_zi_bootstraps01DEC22.RData') # intermittent save if case crashes
  save(optima, file = 'temprange_zi_optimaCI01DEC22.RData') #intermittent save if case crashes
  save(warnings, file = 'temprange_zi_bootstrapwarnings01DEC22.RData') # intermittent save if crashes
  ...
  if (is.null(newmodel$result$error) == TRUE) { # if there were no fatal errors...
    if(length(newmodel$warnings) == 0) { # and there were no warnings...
      ...
      pred[ , i + 6] <- predict(newmodel$result$result, newdata = pred, type = "zprob", se = F,  allow.new.levels = T) # save fitted values from bootstrap run as new column
      optima[i, ] <- pred[which.min(pred[[i + 6]]), c(1, i + 6)] # save min fitted value from bootstrap run (i.e., peak of curve - lowest probability of a zero which is highest probability of non-zero) - need to flip later

...
###               CODE FOR CLUSTER BOOTSTRAP WARM EDGE
###                                                                          ###
################################################################################
################################################################################
...
# code for bootstrap run testing warm edge prediction
...
load('edge/warmhypodata.RData') # cluster run only
...
nreps <- 12000 # number of reps - note that lots of these reps will not converge

# empty data frames: 
pred <- expand.grid(temprange_fac = levels(trailing$temprange_fac), pptrange_pos = 0, canopy = "open", 
...
# the looooop
for (i in 1:nreps) {
  save(pred, file = 'edge/warnmdge_pred29JUN23.RData') #intermittent save if case crashes
  save(warnings, file = 'edge/warmedge_warnings29JUN23.RData') # intermittent save if crashes
  print(paste0("Running model ", i)) # keep track of progress
  newmodel <- quietly(safely((refit)))(mhot, simulate(mhot)[[1]]) # bootstrap run that saves warnings (quietly) AND errors (safely) - warnings are convergence issues and errors are fatal (e.g., entire simulated dataset are zeroes)
